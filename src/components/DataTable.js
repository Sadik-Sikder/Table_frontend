...
const DataTableWithChart = () => {
  ...
  const fetchData = async () => {
    try {
      const result = await axios.get('https://sadiksikder.pythonanywhere.com/api/data/');
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (confirmDelete) {
        await axios.delete(`https://sadiksikder.pythonanywhere.com/api/data/${id}/`);
        fetchData();
      }
    } catch (error) {
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`https://sadiksikder.pythonanywhere.com/api/data/${editId}/`, formData);
      setEditId(null);
      setFormData({
        date: '',
        trade_code: '',
